...
class ScheduledBudget(models.Model):
	...
	categorygroup = models.ForeignKey(CategoryGroup, on_delete=models.CASCADE, null=True, blank=True)
	amount = models.DecimalField(max_digits=8, decimal_places=2)
	last_date = models.DateField(help_text='YYYY-MM-DD: Make sure you enter the correct date!')

	def __str__(self):
		return "%s %s %s %s" %(self.categorygroup, self.category, self.amount, self.last_date)

class SpecifiedBudget(models.Model):
	category = models.ForeignKey(Category, on_delete=models.CASCADE, null=True, blank=True)
	categorygroup = models.ForeignKey(CategoryGroup, on_delete=models.CASCADE, null=True, blank=True)
	amount = models.DecimalField(max_digits=8, decimal_places=2)
	date = models.DateField(help_text='YYYY-MM-DD: Make sure you enter the correct date!')

	def __str__(self):
		return "%s %s %s %s" %(self.categorygroup, self.category, self.amount, self.date)
